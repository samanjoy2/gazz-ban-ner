...
def main(train_file_path, validation_file_path, test_file_path, train=False):
    ...
    # selected_layers = [model.electra.encoder.layer[5], model.electra.encoder.layer[11], model.electra.encoder.layer[17]]
    # # Create a new model instance and replace its layers
    # custom_model = model.__class__(config)
    # custom_model.electra.encoder.layer = nn.ModuleList(selected_layers)
...
if __name__ == '__main__':
    main(train_file_path='good_train.jsonl', validation_file_path='good_dev.jsonl', test_file_path = 'good_test.jsonl',train=True)